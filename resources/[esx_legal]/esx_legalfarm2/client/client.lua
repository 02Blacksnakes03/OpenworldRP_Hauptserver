...
AddEventHandler('esx_legalfarm2:hasEnteredMarker', function(zone)
	...
	if action[2] == 'Field' or action[2] == 'Field1' or action[2] == 'Field2' or action[2] == 'Field3' then
		CurrentAction     = zone
	...
	if action[2] == 'Processing' then
		ESX.TriggerServerCallback('esx_legalfarm2:getInventoryItem', function(qtd)
...
RegisterNetEvent('esx_legalfarm2:hasExitedMarker')
AddEventHandler('esx_legalfarm2:hasExitedMarker', function(zone)
	CurrentAction = nil
	ESX.UI.Menu.CloseAll()
	TriggerServerEvent('esx_legalfarm2:stopHarvest')
	TriggerServerEvent('esx_legalfarm2:stopTransform')
	TriggerServerEvent('esx_legalfarm2:stopSell')
end)

-- Drug effect
RegisterNetEvent('esx_legalfarm2:onUse')
AddEventHandler('esx_legalfarm2:onUse', function(drug)
		...
		for k,v in pairs(Config.Drugs) do
			for i,j in pairs(v.Zones) do
				if(GetDistanceBetweenCoords(coords, j.x, j.y, j.z, true) < Config.DrawDistance) then
					if i == 'Dealer' or  i == 'Processing' then
						if NPCs[k] == nil then
...
Citizen.CreateThread(function()
	while true do
		...
		if isInMarker and not hasAlreadyEnteredMarker then
			...
			TriggerEvent('esx_legalfarm2:hasEnteredMarker', currentZone)
		end

		if not isInMarker and hasAlreadyEnteredMarker then
			hasAlreadyEnteredMarker = false
			TriggerEvent('esx_legalfarm2:hasExitedMarker', lastZone)
		end
	end
end)
status = false
	...
	while true do
		...
		if CurrentAction ~= nil then
			...
			if IsControlJustReleased(0, Keys['E']) then
				...
				elseif action[2] == "Field"  or action[2] == "Field1" or action[2] == "Field2"  or action[2] == "Field3" or action[2] == "Field4" then
					ESX.ShowHelpNotification(_U('stop_action', Config.KeyStopAction))
					TriggerServerEvent('esx_legalfarm2:startHarvest', action[1])
					--TaskStartScenarioInPlace(playerPed, 'world_human_gardener_plant', 0, false)
					startEmoteHarvest()
				elseif action[2] == "Processing" then
					TriggerServerEvent('esx_legalfarm2:startTransform', action[1])
					startEmoteProcess()
				elseif action[2] == "Dealer" then
					TriggerServerEvent('esx_legalfarm2:startSell', action[1])
					startEmoteDealer()
				else
					isInZone = false
				end
