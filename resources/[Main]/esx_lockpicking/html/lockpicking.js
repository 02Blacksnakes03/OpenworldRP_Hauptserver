    ...
    Object.defineProperty(this, 'duration', {
      ...
      set: (value) => {
        ...
        if (!_duration) {
          ...
          this.point = 50;
          setTimeout(function(){
            fetch(`https://${GetParentResourceName()}/lockpickingStatus`, {
              method: 'POST',
              headers: {
...
window.addEventListener('message', function(event) {
  if (event.data.type == "enableui") {
	  $(".PicklockGame").css("visibility", "visible");
	}
  if (event.data.type == "disableui") {
	  $(".PicklockGame").css("visibility", "hidden");
	}
});
