...
cloakMenu = NativeUI.CreateMenu("L.S.P.D", "Garderoben", "")
_menuPool:Add(cloakMenu)

armMenu = NativeUI.CreateMenu("L.S.P.D", "Waffenkammer", "")
_menuPool:Add(armMenu)

carMenu = NativeUI.CreateMenu("L.S.P.D", "Garage")
_menuPool:Add(carMenu)

bossMenu = NativeUI.CreateMenu("L.S.P.D", "Unternehmensführung")
_menuPool:Add(bossMenu)

mobMenu = NativeUI.CreateMenu("L.S.P.D", "Interaktionsmenü")
...
AddEventHandler('esx_policejob:onDuty', function()
	...
	ESX.ShowAdvancedNotification("L.S.P.D", "OnDuty gehen", "Sie haben Ihren Dienst in Anspruch genommen!", mugshotStr, 8)
...
AddEventHandler('esx_policejob:offDuty', function()
	local mugshot, mugshotStr = ESX.Game.GetPedMugshot(PlayerPedId())
	ESX.ShowAdvancedNotification("L.S.P.D", "OffDuty gehen", "Sie haben Ihr Dienstende abgeschlossen!", mugshotStr, 8)
    UnregisterPedheadshot(mugshot)
	onDuty = false
end)
...
function AddCloakMenu(menu)
	...
	civil = NativeUI.CreateItem("Nimm dein Dienstende", "Drücken Sie ENTER, um das Ende des Dienstes zu übernehmen")
	service = NativeUI.CreateItem("Nimm deinen Dienst", "Drücken Sie ENTER, um Ihren Dienst in Anspruch zu nehmen")
	separator2 = NativeUI.CreateItem("↓ Outfits", "")
    bullet = NativeUI.CreateItem("Kevlar anziehen", "Drücken Sie ENTER , um ein Kevlar . einzugeben")
	tenu = NativeUI.CreateItem("Zieh dein Service-Outfit an", "Drücke ENTER, um deine Uniform anzuziehen")
...
function AddArmMenu(menu)
	...
	local deposit = NativeUI.CreateItem("Lege deine Waffen ab", "Drücken Sie die EINGABETASTE, um alle Ihre Waffen abzulegen")
	menu:AddItem(deposit)

	local kitBasique = NativeUI.CreateItem("Basis-Kit nehmen", "Drücken Sie ENTER, um ein Basis-Kit zu nehmen")
	kitBasique:RightLabel("→")
	menu:AddItem(kitBasique)

	local kitIntervention = NativeUI.CreateItem("Nehmen Sie ein Interventionsset", "Drücken Sie die EINGABETASTE, um ein Interventionsset abzuholen")
	...
	menu.OnItemSelect = function(sender, item, index)
		if item == deposit then
			...
			ESX.ShowAdvancedNotification("L.S.P.D", "Bob", "Danke, ich kümmere mich um das Aufräumen", "CHAR_MP_ARMY_CONTACT", 8)
		end

		if item == kitBasique then
			ESX.ShowAdvancedNotification("Bob", "Hier ist dein Basis-Kit !", "Vergiss nicht, es mir am Ende deiner Schicht zurückzugeben!", "CHAR_MP_ARMY_CONTACT", 8)
...
function AddCadetMenu(menu)
	cadetmenu = _menuPool:AddSubMenu(menu, "Kadett")
	cadetmenu.Item:RightLabel("→")

	local scorcher = NativeUI.CreateItem("Patrouillenfahrrad", "Drücken Sie ENTER, um das Menü zu verlassen 'scorcher'")
	scorcher:RightLabel("→")
	cadetmenu.SubMenu:AddItem(scorcher)
	_menuPool:CloseAllMenus()
	local cruiser = NativeUI.CreateItem("Polizeikreuzer", "Drücken Sie ENTER, um das Menü zu verlassen  'police'")
	cruiser:RightLabel("→")
	...
	cadetmenu.SubMenu.OnItemSelect = function(sender, item, index)
		if item == cruiser then
			ESX.ShowAdvancedNotification("Jack", "Hier ist dein Fahrzeug !", "Vergessen Sie nicht, es am Ende Ihrer Schicht wegzuräumen!", "CHAR_JIMMY_BOSTON", 8)
			spawnCar("police")
...
function AddOfficierMenu(menu)
	officiermenu = _menuPool:AddSubMenu(menu, "Offizier")
	officiermenu.Item:RightLabel("→")

	local buffalo = NativeUI.CreateItem("Polizeibüffel", "Drücken Sie ENTER, um das Menü zu verlassen 'police2'")
	buffalo:RightLabel("→")
	officiermenu.SubMenu:AddItem(buffalo)
	local interceptor = NativeUI.CreateItem("Abfangjäger der Polizei", "Drücken Sie ENTER, um das Menü zu verlassen 'police3'")
	interceptor:RightLabel("→")
	officiermenu.SubMenu:AddItem(interceptor)
	local moto = NativeUI.CreateItem("Polizeimotorrad", "Drücken Sie ENTER, um das Menü zu verlassen 'policeb'")
	moto:RightLabel("→")
...
function AddTransportMenu(menu)
	transportmenu = _menuPool:AddSubMenu(menu, "Transportfahrzeuge", "Zugänglich von Sergeant")
	transportmenu.Item:RightLabel("→")

	local riot = NativeUI.CreateItem("Gepanzertes Fahrzeug der Polizei", "Drücken Sie ENTER, um das Menü zu verlassen  'riot'")
	riot:RightLabel("→")
	transportmenu.SubMenu:AddItem(riot)
	local fourgon = NativeUI.CreateItem("Transporter", "Drücken Sie ENTER, um das Menü zu verlassen'policet'")
	fourgon:RightLabel("→")
	transportmenu.SubMenu:AddItem(fourgon)
	local pbus = NativeUI.CreateItem("Bus für den Strafvollzug", "Drücken Sie ENTER, um das Menü zu verlassen 'pbus'")
	pbus:RightLabel("→")
	transportmenu.SubMenu:AddItem(pbus)
	local riot2 = NativeUI.CreateItem("Anti-Aufruhr-Van", "Drücken Sie ENTER, um das Menü zu verlassen 'riot2'")
	riot2:RightLabel("→")
...
function AddUnmarkedMenu(menu)
	unmarkedmenu = _menuPool:AddSubMenu(menu, "Undercover Fahrzeuge")
	unmarkedmenu.Item:RightLabel("→")

	local cruiser2 = NativeUI.CreateItem("Cruiser undercover", "Drücken Sie ENTER, um das Menü zu verlassen 'police4'")
	cruiser2:RightLabel("→")
	unmarkedmenu.SubMenu:AddItem(cruiser2)
	local buffalo2 = NativeUI.CreateItem("Buffalo undercover", "Drücken Sie ENTER, um das Menü zu verlassen 'fbi'")
	buffalo2:RightLabel("→")
	unmarkedmenu.SubMenu:AddItem(buffalo2)
	local granger2 = NativeUI.CreateItem("Granger undercover", "Drücken Sie ENTER, um das Menü zu verlassen 'fbi2'")
	granger2:RightLabel("→")
	unmarkedmenu.SubMenu:AddItem(granger2)
	local sultan = NativeUI.CreateItem("Véhicule de formation", "Drücken Sie ENTER, um das Menü zu verlassen 'sultan'")
	sultan:RightLabel("→")
	unmarkedmenu.SubMenu:AddItem(sultan)

	unmarkedmenu.SubMenu.OnItemSelect = function(sender, item, index)
		if item == cruiser2 then
			ESX.ShowAdvancedNotification("Jack", "Hier ist dein Fahrzeug !", "Vergiss nicht, es am Ende deiner Schicht wegzuräumen !", "CHAR_JIMMY_BOSTON", 8)
			spawnCar("police4")
			_menuPool:CloseAllMenus()
...
function AddMobMenu(menu)
	citizen = _menuPool:AddSubMenu(menu, "Bürger", "")
	citizen.Item:RightLabel("→")
	vehicle = _menuPool:AddSubMenu(menu, "Fahrzeuge", "")
	vehicle.Item:RightLabel("→")
	object = _menuPool:AddSubMenu(menu, "Objekte", "")
	object.Item:RightLabel("→")

	interaction = NativeUI.CreateItem("↓ Interaktionen mit der Person", "")
	interaction:RightLabel("↓")
	search = NativeUI.CreateItem("Durchsuche eine Person", "Drücken Sie ENTER, um die Person vor Ihnen zu durchsuchen")
	search:RightLabel("→")
	handcuff = NativeUI.CreateItem("Person mit Handschellen fesseln", "Drücken Sie die EINGABETASTE, um der Person vor Ihnen Handschellen anzulegen")
	handcuff:RightLabel("→")
	drag = NativeUI.CreateItem("Nimm die Person am Arm", "Drücken Sie ENTER, um die Person vor Ihnen am Arm zu nehmen")
	drag:RightLabel("→")
	papier = NativeUI.CreateItem("↓ Papiere prüfen", "")
	papier:RightLabel("↓")
	fine = NativeUI.CreateItem("Geben Sie eine Geldstrafe", "Drücken Sie ENTER, um der Person vor Ihnen eine Geldstrafe zu geben")
	fine:RightLabel("→")
	unpaid_bills = NativeUI.CreateItem("Unbezahlte Strafzetel anzeigen", "Drücken Sie ENTER, um zu überprüfen, ob die Person vor Ihnen unbezahlte Rechnungen hat")
	unpaid_bills:RightLabel("→")
	license_check = NativeUI.CreateItem("Lizenzen anzeigen", "Drücken Sie ENTER, um die Lizenzen der Person vor Ihnen anzuzeigen")
	...
	vehicle_info = NativeUI.CreateItem("Überprüfen Sie das Fahrzeugschild "," Drücken Sie ENTER, um das Fahrzeugschild über das Steuergerät zu überprüfen")
	pick_lock = NativeUI.CreateItem("Fahrzeug abholen", "Drücken Sie ENTER, um das Fahrzeug auszuwählen")
	impound = NativeUI.CreateItem("Beschlagnahme des Fahrzeugs", "NUR WENN KEIN MECANO VERFÜGBAR")
	database = NativeUI.CreateItem("Teller suchen über die Zentrale", "Drücken Sie ENTER, um mit dem Bedienfeld zu kommunizieren")
	...
	cone = NativeUI.CreateItem("Platziere einen Verkehrskegel", "Drücken Sie ENTER, um einen Kegel zu platzieren")
	barrier = NativeUI.CreateItem("Platziere eine Sicherheitsbarriere", "Drücken Sie ENTER, um eine Barriere zu platzieren")
	herse = NativeUI.CreateItem("Platziere eine Egge", "Drücken Sie ENTER, um eine Egge zu platzieren")
    ...
    while true do
		...
		if IsControlJustPressed(1,167) and PlayerData.job and PlayerData.job.name == 'police' then
			if onDuty then
				...
				local mugshot, mugshotStr = ESX.Game.GetPedMugshot(PlayerPedId())
				ESX.ShowAdvancedNotification("L.S.P.D", "Unmögliche Aktion", "Dafür musst du OnDuy gehen", mugshotStr, 8)
				UnregisterPedheadshot(mugshot)
			end
		end
        ...
        for k in pairs(arm) do
            ...
            if dist <= 1.2 and onDuty and PlayerData.job and  PlayerData.job.name == 'police' then
                ESX.ShowHelpNotification("Drücken Sie auf ~INPUT_PICKUP~ um die Waffenkammer zu ~b~l'öffnen~s~")
		...
		for k in pairs(cloak) do
            ...
            if dist <= 1.2 and PlayerData.job and PlayerData.job.name == 'police' then
                ESX.ShowHelpNotification("Drücken sie auf~INPUT_PICKUP~ um die Umkleide zu ~b~öffnen~s~")
				if IsControlJustPressed(1,38) then 
		...
		for k in pairs(garage) do
            ...
            if dist <= 1.2 and onDuty and PlayerData.job and PlayerData.job.name == 'police' then
                ESX.ShowHelpNotification("Drücken sie auf ~INPUT_PICKUP~ um die Garage zu ~b~öffnen~s~")
				if IsControlJustPressed(1,38) then 
		...
		for k in pairs(boss) do
            ...
            if dist <= 1.2 and onDuty and PlayerData.job and PlayerData.job.name == 'police' and PlayerData.job.grade_name == 'boss' then
                ESX.ShowHelpNotification("Drücken sie auf ~INPUT_PICKUP~ um auf den Computer ~b~l'zuzugreifen~s~")
				if IsControlJustPressed(1,38) then 
    ...
    while true do
		...
		for k in pairs(del) do
            ...
            if dist <= 5.5 and onDuty and PlayerData.job and PlayerData.job.name == 'police' and IsPedInAnyVehicle(PlayerPedId(-1), false) then
                ESX.ShowHelpNotification("Drücken sie auf ~INPUT_PICKUP~ zum eingeben des  ~b~Fahrzeugs~s~ in der ~b~Garage~s~")
				if IsControlJustPressed(1,38) then 			
